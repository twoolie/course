...
flatMap ::
  ...
  -> List a
  -> List b
flatMap f (l:.ls) = f l ++ flatMap f ls
flatMap _ Nil     = Nil
...
seqOptional (Full v :. os ) = case seqOptional os of
  Full vs -> Full $ v :. vs
  Empty -> Empty
